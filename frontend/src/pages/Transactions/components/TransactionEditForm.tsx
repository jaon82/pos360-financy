...
import { GET_TRANSACTION } from '@/lib/graphql/queries/Transaction';
...
export default function TransactionEditForm({
  ...
  const [getTransaction, { data: transactionData }] =
    useLazyQuery<TransactionData>(GET_TRANSACTION);
  ...
  useEffect(() => {
    if (id) {
      getTransaction({ variables: { getTransactionId: id } });
    }
  }, [id, getTransaction]);
  ...
  return (
    <Dialog open={open} onOpenChange={handleToggleForm}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            {id ? 'Editar' : 'Nova'} transação {transaction?.id}
          </DialogTitle>
